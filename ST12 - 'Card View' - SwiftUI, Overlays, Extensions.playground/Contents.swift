...
struct ContentView: View {
    var body: some View {
        Text("You are a good person")
            .cardStyle(color: Color.cyan, padding: 20)
    }
}

PlaygroundPage.current.setLiveView(ContentView())

struct ReusableCard: ViewModifier {
    var color: Color
    var padding: CGFloat
        ...
        content
            ...
            .background {
                RoundedRectangle(cornerSize: CGSize(width: 10, height: 10))
                    .foregroundStyle(color.gradient.shadow(.inner(radius: 3)))
        }
    }
}

extension View {
    func cardStyle(color: Color, padding: CGFloat) -> some View {
        modifier(ReusableCard(color: color, padding: padding))
    }
}
