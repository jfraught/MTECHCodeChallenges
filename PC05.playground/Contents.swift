...
func spotsFor(_ n: Int, in arr: [Int]) -> Int {
    var spots = 0
    var currentSpot = 1
    var isEven = n % 2 == 0
    ...
    for i in 0..<arr.count - 1 {
        if isEven {
            if arr[i] % 2 == 0 || arr[i + 1] == 0 {
                print("Even \(arr[i]) \(n) \(arr[i + 1])")
                spots += 1
            }
        } else {
            if arr[i] % 2 != 0 || arr[i + 1] % 2 != 0 {
                print("\(arr[i + 1] % 2 != 0)")
                print("Odd \(arr[i]) \(n) \(arr[i + 1])")
                spots += 1
            }
        }
    }
...
spotsFor(9, in: [0, 4, 6, 8])
spotsFor(12, in: [0, 4, 6, 8])
spotsFor(7, in: [4, 4, 4, 4, 5])
